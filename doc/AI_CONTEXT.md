# AI\_CONTEXT.md — mp\_Net-Light 統一專案說明文件
...
**mp\_Net-Light** 是一個高效能 **Server ⇄ MicroPython Client**（ESP32 系列 MCU）傳輸控制系統。
...
[Stream Parser] → [Schema Decoder] → [Dispatcher] → [Action Layer]
     ↑                                                    ↓
     |_____________ [CRC32 驗證 + SOF 重同步] ____________|
```
...
| CRC32 | 4B | CRC32 (binascii/ubinascii.crc32)，覆蓋 VER..DATA |
...
4. **max\_len 保護**：避免誤同步讀到超大 LEN 造成記憶體溢出
...
| str\_u16len  | 字串（前綴 2B 長度） | 檔案名、路徑       |
| bytes\_fixed | 固定長度 bytes   | SHA256 (32B) |
| bytes\_rest  | 吃掉剩餘所有 bytes | 檔案塊、像素資料     |
...
- `Proto.pack(cmd, payload=b"", addr=0xFFFF) -> bytes`（VER=4）
- `StreamParser.feed(data)` / `.pop() -> (ver, addr, cmd, payload_view)`
- CRC32 驗證（覆蓋 VER..DATA）
...
| 0x1002 | SLAVE\_ANNOUNCE | MCU → Server | `slave_id(str)` `pixel_count(u16)` `hw_version(str)` | 從機回報身份與硬體資訊 |
...
| CMD    | 名稱                  | 方向           | Payload                      | 說明                |
| ------ | ------------------- | ------------ | ---------------------------- | ----------------- |
| 0x1101 | STATUS\_GET         | Server → MCU | `query_type(u8)`             | 請求狀態 (0=全部, 1=精簡) |
| 0x1102 | STATUS\_RSP         | MCU → Server | `status_json(str)`           | 回傳 JSON 格式狀態      |
| 0x1103 | STATUS\_UPDATE      | Server → MCU | `config_json(str)`           | 更新配置 (亮度/模式等)     |
| 0x1104 | STATUS\_UPDATE\_ACK | MCU → Server | `success(u8)` `message(str)` | 配置更新結果回報          |
...
| 0x1202 | HEARTBEAT\_ACK | Server → MCU | `server_time(u32)` `success(u8)`                                    | Server 確認存活 |
...
| CMD    | 名稱            | 方向           | Payload                                                        | 說明                   |
| ------ | ------------- | ------------ | -------------------------------------------------------------- | -------------------- |
| 0x1205 | FS\_TREE\_GET | Server → MCU | `path(str)` `max_depth(u8)` `include_size(u8)`                 | 請求目錄樹 (單包回傳)         |
| 0x1206 | FS\_TREE\_RSP | MCU → Server | `path(str)` `tree(str)`                                        | 回傳文字格式目錄樹            |
| 0x1213 | FS\_SNAP\_GET | Server → MCU | `path(str)` `out_path(str)` `max_depth(u8)` `include_size(u8)` | 生成 JSON 快照 (FILE 傳輸) |
...
| CMD    | 名稱          | 方向 | Payload                                                                      | 說明            |
| ------ | ----------- | -- | ---------------------------------------------------------------------------- | ------------- |
| 0x2001 | FILE\_BEGIN | 雙向 | `file_id(u16)` `total_size(u32)` `chunk_size(u16)` `sha256(32B)` `path(str)` | 開始檔案傳輸        |
| 0x2002 | FILE\_CHUNK | 雙向 | `file_id(u16)` `offset(u32)` `data(bytes_rest)`                              | 傳輸檔案塊         |
| 0x2003 | FILE\_END   | 雙向 | `file_id(u16)`                                                               | 傳輸完成通知        |
| 0x2004 | FILE\_ACK   | 雙向 | `file_id(u16)` `offset(u32)`                                                 | 確認收到 (用於斷點續傳) |
...
| CMD    | 名稱            | 方向           | Payload                                  | 說明     |
| ------ | ------------- | ------------ | ---------------------------------------- | ------ |
| 0x3001 | STREAM\_START | Server → MCU | `fps(u8)`                                | 開始串流模式 |
| 0x3002 | STREAM\_STOP  | Server → MCU | (空)                                      | 停止串流   |
| 0x3003 | STREAM\_FRAME | Server → MCU | `frame_id(u32)` `pixel_data(bytes_rest)` | 推送像素幀  |
...
協議層已切換為 CRC32（4 bytes），直接使用 `ubinascii/binascii.crc32` 以降低 CPU 成本。
...
**現有協議**：
- 封包格式：SOF(2)=b'NL' + VER(1)=4 + ADDR(2) + CMD(2) + LEN(2) + DATA + CRC32(4)
- CRC32 覆蓋 VER..DATA
...
- /tasks：任務層（network_io.py, network_decode.py, render.py, web_ui.py）
