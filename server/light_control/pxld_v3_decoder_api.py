    ...
    def get_slave_rgbw_bytes(self, frame_id: int, slave_id: int) -> bytes:
        ...
        for s in frame["slaves"]:
            if s.slave_id == slave_id:
                ...
                break
        if slave is None:
            raise ValueError(f"Frame {frame_id} 找不到 slave_id={slave_id}")
        
        pixel_data: bytes = frame["pixel_data"]
        start = slave.data_offset
        end = start + slave.data_length
        raw = pixel_data[start:end]
        
        # 修正：處理黑色顯示問題
        # 將 RGBW 數據轉換為 bytearray 以便修改
        rgbw_bytes = bytearray(raw)
        
        # 修正方案：如果 W 值過大，降低其影響
        for i in range(0, len(rgbw_bytes), 4):
            if i + 3 < len(rgbw_bytes):
                r, g, b, w = rgbw_bytes[i], rgbw_bytes[i+1], rgbw_bytes[i+2], rgbw_bytes[i+3]
                
                # 修正1: 如果 RGB 都是 0，且 W > 0，減少 W 值使顯示更暗
